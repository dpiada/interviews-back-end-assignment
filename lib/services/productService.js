...
class ProductService {
     ...
     * @param {number} [category_id] - Optional - The ID of the category to filter products by.
     ...
     * @returns {Promise<Array>} A promise that resolves to an array of products. Each product object includes the name, price, photo path, quantity, availability, and category name.
    */
    async listAllProducts({ page = 0, size = 10, category_id, search, availability }) {
        ...
        if (category_id) {
            productsQuery = productsQuery.where({ category_id });
        }
